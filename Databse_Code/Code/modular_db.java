...
public class modular_db {
	...
	private static boolean ConnectJDBC() 
	{
		try{
			...
			st=con.createStatement();
				...
				{System.out.println("Could not connect to the database");e.printStackTrace(); return false;}
	...
	private static boolean SetSize(double a)
	{
		...
		String size1="PRAGMA max_page_count="+pc;   //97666*1024=Approximately 100 MB
		
		
		try {											//page_size=1024
			st.execute(size1);
			} catch (SQLException e) {System.out.println("Could not set the page size"); e.printStackTrace();return false; }
	...
	private static boolean CreateLog()
	{
		String sql="CREATE TABLE Process_info" +
			...
			} catch (SQLException e) {System.out.println("Could not create the table"); e.printStackTrace();return false;}
	...
	public static  void InsertIntoLog(String a[]) 
		{
			String LoggingTime=new SimpleDateFormat("yyyyMMdd_HHmmss").format(Calendar.getInstance().getTime());
				...
				try{
					...
					}catch(Exception e){BackupDb(LoggingTime,a[0]);}
